        ...
        do l = 1, num_nodes_y, 1
            ...
            do j = 1, dim, 1
                ...
                do jj =1, num_neighbors(j), 1
                    ut = ut + (x(k,neighbors(i0+jj)) - y(l,neighbors(i0+jj)))**2
                end do
                ut = sqrt(ut)/sig
                if (ord > 4) then
                    ut = ut * ut
...
subroutine applylocalkdifft(x, y, a1, a2, sig, ord, neighbors, num_neighbors, num_nodes, num_nodes_y, dim, tot_neighbors, na, f)
    ...
    !$omp parallel do private(k,l,ut,lpt,Kh_diff,df,ii,jj,i0,j) shared &
    ...
    do k = 1, num_nodes, 1
        ...
        do l = 1, num_nodes_y, 1
            ...
            do j = 1, dim, 1
                ...
                do jj =1, num_neighbors(j), 1
                    ii = neighbors(i0+jj)
                    !print *, j, num_neighbors(j), i0, jj, ii
                    df(ii) = df(ii) + Kh_diff * 2*(x(k,ii)-y(l,ii))* sum(a1(:,k,j)*a2(:,l,j))
                end do
                i0 = i0 + num_neighbors(j)
            end do
        end do
